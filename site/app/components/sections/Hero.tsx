...
// Platform logos for the rotating headline
function WhatsAppLogo({ className = "h-[1.1em] w-[1.1em]" }: { className?: string }) {
  return (
...
function TelegramLogo({ className = "h-[1.1em] w-[1.1em]" }: { className?: string }) {
  return (
...
function SlackLogo({ className = "h-[1.1em] w-[1.1em]" }: { className?: string }) {
  return (
...
function AppleLogo({ className = "h-[1.1em] w-[1.1em]" }: { className?: string }) {
  return (
...
function BrowserLogo({ className = "h-[1.1em] w-[1.1em]" }: { className?: string }) {
  return (
...
function EmailLogo({ className = "h-[1.1em] w-[1.1em]" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className={className}>
...
// Rotating items: logo + label, synced with 6 Remotion showcase scenes
...
const rotatingItems = [
  ...
  <><SlackLogo /> Slack</>,
  <><BrowserLogo /> your browser</>,
  <><EmailLogo /> your inbox</>,
  <><AppleLogo /> your Mac</>,
]

export function Hero() {
  return (
    <section className="relative flex flex-col items-center justify-center overflow-hidden pt-28 sm:pt-32 pb-20">
      {/* Layer 1: Aurora gradient */}
      <AuroraBackground className="absolute inset-0" />

      {/* Layer 2: Remotion orb animation */}
      <SectionPlayer
        component={HeroOrb}
        opacity={0.35}
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-6 text-center">
        ...
        {/* Headline with rotating platform logos */}
        <motion.h1
        ...
        >
          <span className="text-text">Your personal AI agent on</span>
          <br />
          <span className="bg-gradient-to-r from-accent via-purple to-accent bg-clip-text text-transparent bg-[length:200%_auto] animate-gradient">
            <FlipNodes
              items={rotatingItems}
              duration={SCENE_DURATION_MS}
              className="bg-gradient-to-r from-accent via-purple to-accent bg-clip-text text-transparent"
            />
          </span>
        </motion.h1>
        ...
        >
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.98 }}
            className="inline-flex items-center gap-3 rounded-lg border border-border px-7 py-3.5 text-base font-medium text-text-secondary hover:text-text hover:border-border-hover transition-all"
          >
            See what it does
